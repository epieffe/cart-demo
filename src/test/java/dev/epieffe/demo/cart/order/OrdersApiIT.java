...
public class OrdersApiIT {
	...
	void createOrder_shouldReturnOrder() throws Exception {
		...
		mockMvc.perform(post("/api/orders").contentType(MediaType.APPLICATION_JSON).content(json))
				...
				.andExpect(jsonPath("$.netPrice").value(1475.41))
				.andExpect(jsonPath("$.vatAmount").value(324.58))
				...
				.andExpect(jsonPath("$.products[0].netPrice").value(1311.48))
				.andExpect(jsonPath("$.products[0].vatAmount").value(288.52))
				...
				.andExpect(jsonPath("$.products[1].netPrice").value(163.93))
				.andExpect(jsonPath("$.products[1].vatAmount").value(36.06))
	...
	void getOrder_shouldPersistOrderAfterProductIsDeleted() throws Exception {
		...
		mockMvc.perform(get("/api/orders/" + id))
				...
				.andExpect(jsonPath("$.products[0].name").value("Samsung Galaxy S24"))
				.andExpect(jsonPath("$.products[0].totalPrice").value(1600))
				.andExpect(jsonPath("$.products[0].netPrice").isNumber())
